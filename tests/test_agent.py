...
def test_critical_severity_triggers_docker_diagnostics_and_notify(
    ...
    monkeypatch.setenv("ENABLE_DOCKER_MONITORING", "1")
    ...
    assert final.get("docker_context", {}).get("ok") is True
    assert notify_calls["n"] == 1
    assert final.get("notify_result", {}).get("ok") is True


def test_docker_diagnostics_skipped_when_monitoring_disabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    pytest.importorskip("langgraph")
    ...
    monkeypatch.setenv("USE_LANGCHAIN_AGENT", "0")
    monkeypatch.setenv("PROBE_URLS", "")
    monkeypatch.setenv("ENABLE_DOCKER_MONITORING", "0")
    ...
    monkeypatch.setattr("agent.orchestrator.collect_container_diagnostics", fake_docker_diag)
    monkeypatch.setattr(
        "agent.orchestrator.send_push_notification",
        lambda **kwargs: {"ok": True, "status_code": 200},
    )

    final = run_cycle({"docker_log_errors": []})

    assert docker_calls["n"] == 0
    assert final.get("docker_context") is None
